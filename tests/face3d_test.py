...
def test_face3d_split_through_holes():
    ...
    center_splits = face.split_through_hole_center_lines(0.01)
    assert len(center_splits) > 1
...
def test_group_by_coplanarity():
    ...
    bound_pts3 = [Point3D(6, 6, 0.45), Point3D(7, 6, 0.4), Point3D(7, 7, 0.4), Point3D(6, 7, 0.45)]
    ...
    grouped_faces = Face3D.group_by_coplanarity(all_faces, 0.01)
    assert len(grouped_faces) == 2
    assert len(grouped_faces[0]) in (2, 1)
    assert len(grouped_faces[1]) in (2, 1)

    grouped_faces = Face3D.group_by_coplanarity(all_faces, 0.75)
    assert len(grouped_faces) == 1
    assert len(grouped_faces[0]) == 3

    grouped_faces = Face3D.group_by_coplanarity(all_faces, 0.75, math.pi / 180)
    assert len(grouped_faces) == 2
    assert len(grouped_faces[0]) in (2, 1)
    assert len(grouped_faces[1]) in (2, 1)
