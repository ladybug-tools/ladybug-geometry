...
import logging
import heapq
from tests.test_data.euclid import *
...
# From LB to ensure consistency for edge equality checks
def _lbeq(line1, line2):
	return isinstance(line2, LineSegment2) and \
		   (hash(line1.p), hash(Point2(line1.v.x,line1.v.y))) == \
		   (hash(line2.p), hash(Point2(line2.v.x,line1.v.y)))
	...
	def next_event(self):
		...
		if self.is_reflex:
			...
			for iii, edge in enumerate(self.original_edges):
				if _lbeq(edge.edge, self.edge_left) or _lbeq(edge.edge, self.edge_right):
					continue
