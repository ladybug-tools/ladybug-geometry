...
from ladybug_geometry.geometry3d.cone import Cone3D

import math


def test_cone_init():
    """Test the initalization of Cone3D objects and basic properties."""
    vertex = Point3D(2, 0, 2)
    axis = Vector3D(0, 2, 2)
    angle = 0.7
    c = Cone3D(vertex, axis, angle)
...
def test_equality():
    """Test the equality of Cone3D objects."""
    cone = Cone3D(Point3D(2, 0, 2), Vector3D(0, 2, 2), 0.7)
...
def test_cone_to_from_dict():
    """Test the Cone3D to_dict and from_dict methods."""
    c = Cone3D(Point3D(4, 0.5, 2), Vector3D(1, 0, 2.5), 0.7)
    d = c.to_dict()
    c = Cone3D.from_dict(d)
...
def test_cone_duplicate():
    """Test the Cone3D duplicate method."""
    c = Cone3D(Point3D(0, 0.5, 2), Vector3D(1, 0.5, 2.5), 0.75)
    test = c.duplicate()
...
def test_cone_rotate():
    """Test the Cone3D rotate method."""
    c = Cone3D(Point3D(2, 0, 2), Vector3D(1, 0.5, 2.5), 0.75)
...
def test_cone_rotate_xy():
    """Test the Cone3D rotate_xy method."""
    c = Cone3D(Point3D(1, 0, 3), Vector3D(1, 2, 0), 0.5)
